...
-- Generated flavor catalog for AnniversaryAchievements v2.3.53.
...
local DEFINITION_ACHIEVEMENT_COUNTER = 517
local DEFINITION_CATEGORY_COUNTER = 35
local DEFINITION_CRITERIA_COUNTER = 2840


-- Classic-continent rare data imported from AnniversaryAchievementsPlus 1.15.
-- Entry format: { npcID, factionGate, clientGate }
-- Visible names are resolved exclusively through RARE_NPC_<npcID> locale keys.
-- factionGate: "A" = Alliance only, "H" = Horde only, nil = both
-- clientGate : "era" = Classic Era only, "tbc" = TBC Anniversary only, nil = both
local CLASSIC_RARE_ENTRIES = {
    -- Eastern Kingdoms
    {3586},
    {3872},
    {6228},
...
local function IsClassicRareEntryAvailable(entry)
    local factionGate = entry[2]
    if factionGate and factionGate ~= factionKey then return false end

    local clientGate = entry[3]
    if clientGate == "era" then return not ns.IsTBCAnniversary end
    if clientGate == "tbc" then return ns.IsTBCAnniversary == true end
    return true
end

local function BuildClassicRareCriteria(criteriaBaseID)
    local result = {}
    for sourceIndex, entry in ipairs(CLASSIC_RARE_ENTRIES) do
        if IsClassicRareEntryAvailable(entry) then
            result[#result + 1] = {
                ["coinFormatter"] = false,
                ["data"] = {
                    [1] = entry[1],
                },
                -- IDs are derived from the unfiltered source index so they stay
                -- stable across faction and client variants.
                ["id"] = criteriaBaseID + sourceIndex,
                ["name"] = "RARE_NPC_" .. entry[1],
                ["type"] = TYPE.KILL_NPC,
            }
        end
    end
...
-- Two-stage Classic-continent rare series. Both criteria sets are registered
-- immediately; the series relation controls visibility only. Consequently the
-- kill that completes "One of a Kind" is already stored for "Extremely Rare".
achievementDefinitions[651] = {
    ["anyCompletable"] = true,
    ["categoryID"] = 6,
    ["criteria"] = BuildClassicRareCriteria(2068),
    ["description"] = "AD_ONE_OF_A_KIND",
    ["icon"] = "-INV_Misc_MonsterClaw_02",
    ["id"] = 651,
    ["name"] = "AN_ONE_OF_A_KIND",
    ["nextID"] = 652,
    ["points"] = 10,
}

achievementDefinitions[652] = {
    ["anyCompletable"] = false,
    ["categoryID"] = 6,
    ["criteria"] = BuildClassicRareCriteria(2454),
    ["description"] = "AD_EXTREMELY_RARE",
    ["icon"] = "-Spell_Shadow_DeathScream",
    ["id"] = 652,
    ["name"] = "AN_EXTREMELY_RARE",
    ["points"] = 10,
    ["previousID"] = 651,
    ["rewardText"] = "TITLE_BUILTIN_THE_ELUSIVE_LABEL",
}
